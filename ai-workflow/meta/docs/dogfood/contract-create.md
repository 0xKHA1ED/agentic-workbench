# Dogfood: CREATE mode template (contract-create)

- **Node:** `contract-create` (work) — fragment `fragments/spec-contract.yaml`
...
Selected `contract-create` from the Cockpit weak filter, opened the unified Node HUD
(`workflow_node_status` / `GET /api/node`), ran its VERIFY end-to-end, observed
the module pass (exit 0), then promoted it via
`project_tree.py propose meta set-status contract-create strong` → human apply.

# Dogfood: Claims JSON format (discovery-claims-schema)

- **Node:** `discovery-claims-schema` (work) — fragment `fragments/spec-discovery.yaml`
...
Selected `discovery-claims-schema` from the Cockpit weak filter, opened the unified Node HUD
(`workflow_node_status` / `GET /api/node`), ran its VERIFY end-to-end, observed
the module pass (exit 0), then promoted it via
`project_tree.py propose meta set-status discovery-claims-schema strong` → human apply.

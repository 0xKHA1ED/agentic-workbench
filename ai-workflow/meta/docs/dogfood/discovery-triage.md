# Dogfood: review — y/n/s/e per claim (discovery-triage)

- **Node:** `discovery-triage` (work) — fragment `fragments/spec-discovery.yaml`
...
Selected `discovery-triage` from the Cockpit weak filter, opened the unified Node HUD
(`workflow_node_status` / `GET /api/node`), ran its VERIFY end-to-end, observed
the module pass (exit 0), then promoted it via
`project_tree.py propose meta set-status discovery-triage strong` → human apply.

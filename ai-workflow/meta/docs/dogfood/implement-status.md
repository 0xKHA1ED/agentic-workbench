# Dogfood: spec_approved → done → strong flow (implement-status)

- **Node:** `implement-status` (work) — fragment `fragments/implement.yaml`
...
Selected `implement-status` from the Cockpit weak filter, opened the unified Node HUD
(`workflow_node_status` / `GET /api/node`), ran its VERIFY end-to-end, observed
the module pass (exit 0), then promoted it via
`project_tree.py propose meta set-status implement-status strong` → human apply.

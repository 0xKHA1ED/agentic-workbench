# Dogfood (rollup): project-tree skill (orient-skill)

- **Node:** `orient-skill` (group) — fragment `fragments/orient.yaml`
...
- **Evidence:** `tests.test_meta_strong` asserts the whole composed meta tree is
  strong (0 non-strong nodes), which entails every descendant of `orient-skill`.

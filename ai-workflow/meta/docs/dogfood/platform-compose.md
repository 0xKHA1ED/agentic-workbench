# Dogfood: Fragment compose + list-fragments (platform-compose)

- **Node:** `platform-compose` (work) — fragment `fragments/platform.yaml`
...
Selected `platform-compose` from the Cockpit weak filter, opened the unified Node HUD
(`workflow_node_status` / `GET /api/node`), ran its VERIFY end-to-end, observed
the module pass (exit 0), then promoted it via
`project_tree.py propose meta set-status platform-compose strong` → human apply.

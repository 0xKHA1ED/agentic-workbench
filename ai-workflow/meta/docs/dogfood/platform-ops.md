# Dogfood: reparent, mark-stale, set-all-weak (platform-ops)

- **Node:** `platform-ops` (work) — fragment `fragments/platform.yaml`
...
Selected `platform-ops` from the Cockpit weak filter, opened the unified Node HUD
(`workflow_node_status` / `GET /api/node`), ran its VERIFY end-to-end, observed
the module pass (exit 0), then promoted it via
`project_tree.py propose meta set-status platform-ops strong` → human apply.

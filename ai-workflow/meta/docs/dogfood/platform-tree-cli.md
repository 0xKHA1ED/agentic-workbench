# Dogfood (rollup): project_tree CLI (platform-tree-cli)

- **Node:** `platform-tree-cli` (group) — fragment `fragments/platform.yaml`
...
- **Evidence:** `tests.test_meta_strong` asserts the whole composed meta tree is
  strong (0 non-strong nodes), which entails every descendant of `platform-tree-cli`.

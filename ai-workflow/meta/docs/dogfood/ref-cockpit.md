# Dogfood (rollup): Cockpit / viewer UX (ref-cockpit)

- **Node:** `ref-cockpit` (group) — fragment `fragments/reference-improvements.yaml`
...
- **Evidence:** `tests.test_meta_strong` asserts the whole composed meta tree is
  strong (0 non-strong nodes), which entails every descendant of `ref-cockpit`.

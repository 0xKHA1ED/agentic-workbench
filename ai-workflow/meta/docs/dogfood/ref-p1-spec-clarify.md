# Dogfood: spec-clarify skill + Cockpit Q&A loop (ref-p1-spec-clarify)

- **Node:** `ref-p1-spec-clarify` (work) — fragment `fragments/reference-improvements.yaml`
...
Selected `ref-p1-spec-clarify` from the Cockpit weak filter, opened the unified Node HUD
(`workflow_node_status` / `GET /api/node`), ran its VERIFY end-to-end, observed
the module pass (exit 0), then promoted it via
`project_tree.py propose meta set-status ref-p1-spec-clarify strong` → human apply.

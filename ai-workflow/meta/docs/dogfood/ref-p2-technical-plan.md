# Dogfood: technical-plan — optional plan.md (HOW) separate from contract (ref-p2-technical-plan)

- **Node:** `ref-p2-technical-plan` (work) — fragment `fragments/reference-improvements.yaml`
...
Selected `ref-p2-technical-plan` from the Cockpit weak filter, opened the unified Node HUD
(`workflow_node_status` / `GET /api/node`), ran its VERIFY end-to-end, observed
the module pass (exit 0), then promoted it via
`project_tree.py propose meta set-status ref-p2-technical-plan strong` → human apply.

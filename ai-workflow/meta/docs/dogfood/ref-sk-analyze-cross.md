# Dogfood: Analyze — claims ↔ contract ↔ VERIFY ↔ IN/OUT consistency (ref-sk-analyze-cross)

- **Node:** `ref-sk-analyze-cross` (work) — fragment `fragments/reference-improvements.yaml`
...
Selected `ref-sk-analyze-cross` from the Cockpit weak filter, opened the unified Node HUD
(`workflow_node_status` / `GET /api/node`), ran its VERIFY end-to-end, observed
the module pass (exit 0), then promoted it via
`project_tree.py propose meta set-status ref-sk-analyze-cross strong` → human apply.

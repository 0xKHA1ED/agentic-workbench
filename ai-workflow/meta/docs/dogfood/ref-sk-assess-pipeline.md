# Dogfood: Assess extension — intake→decide before spec-discovery (ref-sk-assess-pipeline)

- **Node:** `ref-sk-assess-pipeline` (work) — fragment `fragments/reference-improvements.yaml`
...
Selected `ref-sk-assess-pipeline` from the Cockpit weak filter, opened the unified Node HUD
(`workflow_node_status` / `GET /api/node`), ran its VERIFY end-to-end, observed
the module pass (exit 0), then promoted it via
`project_tree.py propose meta set-status ref-sk-assess-pipeline strong` → human apply.

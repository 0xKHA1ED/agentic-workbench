# Dogfood: Phase hooks — before/after clarify, specify, implement (ref-sk-phase-hooks)

- **Node:** `ref-sk-phase-hooks` (work) — fragment `fragments/reference-improvements.yaml`
...
Selected `ref-sk-phase-hooks` from the Cockpit weak filter, opened the unified Node HUD
(`workflow_node_status` / `GET /api/node`), ran its VERIFY end-to-end, observed
the module pass (exit 0), then promoted it via
`project_tree.py propose meta set-status ref-sk-phase-hooks strong` → human apply.

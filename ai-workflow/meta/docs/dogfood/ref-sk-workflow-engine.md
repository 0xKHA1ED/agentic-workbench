# Dogfood: Workflow YAML — daily loop run/resume/gates (ref-sk-workflow-engine)

- **Node:** `ref-sk-workflow-engine` (work) — fragment `fragments/reference-improvements.yaml`
...
Selected `ref-sk-workflow-engine` from the Cockpit weak filter, opened the unified Node HUD
(`workflow_node_status` / `GET /api/node`), ran its VERIFY end-to-end, observed
the module pass (exit 0), then promoted it via
`project_tree.py propose meta set-status ref-sk-workflow-engine strong` → human apply.

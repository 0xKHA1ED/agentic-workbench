# Dogfood: Brainstorm — spike/bounded/architectural + approval gate (ref-sp-brainstorm)

- **Node:** `ref-sp-brainstorm` (work) — fragment `fragments/reference-improvements.yaml`
...
Selected `ref-sp-brainstorm` from the Cockpit weak filter, opened the unified Node HUD
(`workflow_node_status` / `GET /api/node`), ran its VERIFY end-to-end, observed
the module pass (exit 0), then promoted it via
`project_tree.py propose meta set-status ref-sp-brainstorm strong` → human apply.

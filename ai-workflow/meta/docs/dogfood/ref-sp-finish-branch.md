# Dogfood: finishing-a-development-branch — merge/PR ritual (ref-sp-finish-branch)

- **Node:** `ref-sp-finish-branch` (work) — fragment `fragments/reference-improvements.yaml`
...
Selected `ref-sp-finish-branch` from the Cockpit weak filter, opened the unified Node HUD
(`workflow_node_status` / `GET /api/node`), ran its VERIFY end-to-end, observed
the module pass (exit 0), then promoted it via
`project_tree.py propose meta set-status ref-sp-finish-branch strong` → human apply.

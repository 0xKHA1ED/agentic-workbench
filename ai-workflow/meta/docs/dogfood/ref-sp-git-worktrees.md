# Dogfood: using-git-worktrees — isolated implement per node (ref-sp-git-worktrees)

- **Node:** `ref-sp-git-worktrees` (work) — fragment `fragments/reference-improvements.yaml`
...
Selected `ref-sp-git-worktrees` from the Cockpit weak filter, opened the unified Node HUD
(`workflow_node_status` / `GET /api/node`), ran its VERIFY end-to-end, observed
the module pass (exit 0), then promoted it via
`project_tree.py propose meta set-status ref-sp-git-worktrees strong` → human apply.

# Dogfood: subagent-driven-development — per-task implementer + review (ref-sp-subagent-dev)

- **Node:** `ref-sp-subagent-dev` (work) — fragment `fragments/reference-improvements.yaml`
...
Selected `ref-sp-subagent-dev` from the Cockpit weak filter, opened the unified Node HUD
(`workflow_node_status` / `GET /api/node`), ran its VERIFY end-to-end, observed
the module pass (exit 0), then promoted it via
`project_tree.py propose meta set-status ref-sp-subagent-dev strong` → human apply.

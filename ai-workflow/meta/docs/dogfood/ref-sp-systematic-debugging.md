# Dogfood: systematic-debugging — pair with /understand for bugs (ref-sp-systematic-debugging)

- **Node:** `ref-sp-systematic-debugging` (work) — fragment `fragments/reference-improvements.yaml`
...
Selected `ref-sp-systematic-debugging` from the Cockpit weak filter, opened the unified Node HUD
(`workflow_node_status` / `GET /api/node`), ran its VERIFY end-to-end, observed
the module pass (exit 0), then promoted it via
`project_tree.py propose meta set-status ref-sp-systematic-debugging strong` → human apply.

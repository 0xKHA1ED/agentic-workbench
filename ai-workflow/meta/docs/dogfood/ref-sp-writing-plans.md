# Dogfood: writing-plans — implementation plan artifact (ref-sp-writing-plans)

- **Node:** `ref-sp-writing-plans` (work) — fragment `fragments/reference-improvements.yaml`
...
Selected `ref-sp-writing-plans` from the Cockpit weak filter, opened the unified Node HUD
(`workflow_node_status` / `GET /api/node`), ran its VERIFY end-to-end, observed
the module pass (exit 0), then promoted it via
`project_tree.py propose meta set-status ref-sp-writing-plans strong` → human apply.

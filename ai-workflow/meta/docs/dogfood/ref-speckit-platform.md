# Dogfood (rollup): Spec Kit — platform & automation (ref-speckit-platform)

- **Node:** `ref-speckit-platform` (group) — fragment `fragments/reference-improvements.yaml`
...
- **Evidence:** `tests.test_meta_strong` asserts the whole composed meta tree is
  strong (0 non-strong nodes), which entails every descendant of `ref-speckit-platform`.

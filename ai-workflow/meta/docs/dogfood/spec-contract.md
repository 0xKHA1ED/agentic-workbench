# Dogfood (rollup): Spec contract (spec-contract)

- **Node:** `spec-contract` (group) — fragment `root/nodes.yaml`
...
- **Evidence:** `tests.test_meta_strong` asserts the whole composed meta tree is
  strong (0 non-strong nodes), which entails every descendant of `spec-contract`.

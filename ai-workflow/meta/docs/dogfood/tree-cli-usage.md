# Dogfood: Propose / diff / apply workflow (tree-cli-usage)

- **Node:** `tree-cli-usage` (work) — fragment `fragments/orient.yaml`
...
Selected `tree-cli-usage` from the Cockpit weak filter, opened the unified Node HUD
(`workflow_node_status` / `GET /api/node`), ran its VERIFY end-to-end, observed
the module pass (exit 0), then promoted it via
`project_tree.py propose meta set-status tree-cli-usage strong` → human apply.

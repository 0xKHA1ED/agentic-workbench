# Dogfood: Fragment authoring docs (tree-fragments-doc)

- **Node:** `tree-fragments-doc` (work) — fragment `fragments/orient.yaml`
...
Selected `tree-fragments-doc` from the Cockpit weak filter, opened the unified Node HUD
(`workflow_node_status` / `GET /api/node`), ran its VERIFY end-to-end, observed
the module pass (exit 0), then promoted it via
`project_tree.py propose meta set-status tree-fragments-doc strong` → human apply.

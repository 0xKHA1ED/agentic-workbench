# Dogfood: attach-subtree for new product areas (understand-attach-subtree)

- **Node:** `understand-attach-subtree` (work) — fragment `fragments/understand.yaml`
...
Selected `understand-attach-subtree` from the Cockpit weak filter, opened the unified Node HUD
(`workflow_node_status` / `GET /api/node`), ran its VERIFY end-to-end, observed
the module pass (exit 0), then promoted it via
`project_tree.py propose meta set-status understand-attach-subtree strong` → human apply.

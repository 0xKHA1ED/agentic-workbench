# Dogfood: Investigation ritual before spec-discovery (understand-investigate)

- **Node:** `understand-investigate` (work) — fragment `fragments/understand.yaml`
...
Selected `understand-investigate` from the Cockpit weak filter, opened the unified Node HUD
(`workflow_node_status` / `GET /api/node`), ran its VERIFY end-to-end, observed
the module pass (exit 0), then promoted it via
`project_tree.py propose meta set-status understand-investigate strong` → human apply.

# Dogfood: Capture pain on node data.pain (understand-pain)

- **Node:** `understand-pain` (work) — fragment `fragments/understand.yaml`
...
Selected `understand-pain` from the Cockpit weak filter, opened the unified Node HUD
(`workflow_node_status` / `GET /api/node`), ran its VERIFY end-to-end, observed
the module pass (exit 0), then promoted it via
`project_tree.py propose meta set-status understand-pain strong` → human apply.

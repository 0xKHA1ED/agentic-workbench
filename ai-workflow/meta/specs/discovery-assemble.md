# Scope Contract: assemble → scope-contract markdown

- **Node:** `discovery-assemble` (work)
- **Pattern:** `scripts/spec_discovery/assemble.py`

## GOAL

assemble → scope-contract markdown behaves as specified and stays regression-proof: its VERIFY command
passes from a clean checkout and fails if the behavior regresses.

## IN

- The behavior implemented under `scripts/spec_discovery/assemble.py`.

# Scope Contract: Claims JSON format

- **Node:** `discovery-claims-schema` (work)
- **Pattern:** `scripts/spec_discovery/model.py`

## GOAL

Claims JSON format behaves as specified and stays regression-proof: its VERIFY command
passes from a clean checkout and fails if the behavior regresses.

## IN

- The behavior implemented under `scripts/spec_discovery/model.py`.
